...
int main() {               // Основная функция
  system("stty -icanon");  // Считывания консоли в другой режим
  while (1) {
    if (score1 == 21) {  // Условия выйгрыша
      clearScreen();
      printf("Player 1 WIN!\n");
      break;
    }
    ...
    clearScreen();  // Очистка окна
    printf("\n");
    frame_drow();         // Отрисовка поля
    switch (getchar()) {  // Функция управления
      case ' ':
        mv_Ball();
        break;
      case 'a':
        ((rocket_p1)-1 == 1) ? rocket_p1 : ((rocket_p1 -= 1));
        mv_Ball();
        break;
      case 'z':
        ((rocket_p1) + 1 == height - 1) ? rocket_p1 : (rocket_p1 += 1);
        mv_Ball();
        break;
      case 'k':
        ((rocket_p2)-1 == 1) ? rocket_p2 : (rocket_p2 -= 1);
        mv_Ball();
        break;
      case 'm':
        ((rocket_p2) + 1 == height - 1) ? rocket_p2 : (rocket_p2 += 1);
        mv_Ball();
        break;
    }
  }
